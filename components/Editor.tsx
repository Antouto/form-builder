...
export function Editor({
  ...
  const { isOpen: isOpenAddToServer, onOpen: onOpenAddToServer, onClose: onCloseAddToServer } = useDisclosure()
        ...
        {stage === 'openFormType' && <><Text mt={5} align='center' width='100%' fontSize={25} fontFamily='Whitney Bold'>How should users open your form?</Text>
          <VStack align='center' mt={10} width='100%' gap={10}>
            <VStack align='left'>
              ...
              <Box>
                <Box transition='border 0.2s' _hover={{ cursor: 'pointer', border: `2px solid ${openFormType === 'button' ? 'rgb(71, 82, 196)' : '#1E1F22'}` }} onClick={() => {
                  if (openFormType === 'button') setStage('form')
                  setOpenFormType('button')
                }} border={openFormType === 'button' ? '2px solid #5865F2' : '2px solid #2B2D31'} borderRadius='10px'>

                  <svg style={{ margin: '5px' }} viewBox="0 0 192 68" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <Box>
                <Box transition='border 0.2s' _hover={{ cursor: 'pointer', border: `2px solid ${openFormType === 'select_menu' ? 'rgb(71, 82, 196)' : '#1E1F22'}` }} onClick={() => {
                  if (openFormType === 'select_menu') setStage('form')
                  setOpenFormType('select_menu')
                }} border={openFormType === 'select_menu' ? '2px solid #5865F2' : '2px solid #2B2D31'} borderRadius='10px'>

                  <svg style={{ margin: '5px' }} viewBox="0 0 150 53" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {stage === 'submissions' && <><Text mt={5} align='center' width='100%' fontSize={25} fontFamily='Whitney Bold'>Where should {formCreationFeatures.includes('approval_forward_submission') ? 'to be approved ' : ''}submissions be sent?</Text>
          <VStack align='center' mt={5} width='100%' gap={5}>
            <Box width='100%' maxWidth='500px'>
              ...
              <SubmissionChannelIDInput index={0} register={register} errors={formState.errors} watch={watch} fixMessage={fixMessage} currentGuild={currentGuild} getValues={getValues} setValue={setValue} cookieValue={cookieValue} getGuild={getGuild} getGuilds={getGuilds} stage={stage} setCookieValue={setCookieValue} loadingGuild={loadingGuild} setLoadingGuild={setLoadingGuild} onOpenAddToServer={onOpenAddToServer} guilds={guilds} currentGuildID={currentGuildID} setCurrentGuildID={setCurrentGuildID} />
        ...
        <ModalContent bg='grey.dark'>
          ...
          <ModalFooter>
            <Button variant='primary' onClick={() => {
              ...
              const popup = window.open(`https://discord.com/oauth2/authorize?client_id=942858850850205717&permissions=378762431504&integration_type=0&scope=bot+applications.commands&guild_id=${currentGuild}&disable_guild_select=true&response_type=code&redirect_uri=https%3A%2F%2Fcreate.discordforms.app%2Fapi%2Fdiscord%2Fauthorized`, 'popup', 'popup=true,width=485,height=700')
