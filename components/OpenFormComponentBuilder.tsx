...
import { Box, Button, CloseButton, FormLabel, HStack } from '@chakra-ui/react'
  ...
  return (
    <Box width='100%'>
      ...
      {formMessageComponents.map((component, i) => <Collapsible name={component.style === 5 ? 'Link Button' : `Button to open Form ${component.custom_id?.match(/\d+/)?.[0]}`} key={component.id} moveUpButton={component.style === 5 && (i !== 0) && <svg onClick={() => formMessageComponentsMove(i, i-1)} style={{ marginRight: '8px', cursor: 'pointer', transition: 'transform 0.2s', transform: `rotate(0deg)` }} width="16" height="16" viewBox="0 0 16 16" fill="none">
        ...
        {/* @ts-expect-error */}
        <HStack><ButtonBuilder forButton={`message.components.0.components.${i}`} error={errors?.message?.components?.[0]?.components?.[i]?.button?.label} errorURL={errors?.message?.components?.[0]?.components?.[i]?.button?.url} button={getValues(`message.components.0.components.${i}`)} register={register} fix={fixMessage} setValue={setValue} watch={watch} buttonLabelPlaceholder={'Open Form'} /></HStack>
      </Collapsible>)}
      <Button variant='primary' style={{ backgroundImage: 'linear-gradient(to right, rgb(52, 66, 217), rgb(1, 118, 164))' }}  isDisabled={!premium || getValues('message.components.0.components')?.length >= 5} onClick={() => {
